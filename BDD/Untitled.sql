...
INSERT INTO `Companyies` VALUES (1,'Companyia la Bleda','Helena Escobar','651300826','yonkis@hoa.cat','789756445','CCC',NULL);
...
CREATE TABLE `Espectacles` (
  ...
  CONSTRAINT `EspectaclesCompanyies` FOREIGN KEY (`ep_idEspectacle`) REFERENCES `Companyies` (`c_idCompanyia`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `Espectacles` VALUES (1,1,'Espectacle 1','Tècnic 1','Requisit 1','Familiar');
...
--
-- Temporary view structure for view `companyiesespectaclesllistat`
--

DROP TABLE IF EXISTS `companyiesespectaclesllistat`;
/*!50001 DROP VIEW IF EXISTS `companyiesespectaclesllistat`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `companyiesespectaclesllistat` AS SELECT 
 1 AS `c_idCompanyia`,
 1 AS `c_Nom`,
 1 AS `c_Responsable`,
 1 AS `c_Telèfon`,
...
--
-- Final view structure for view `companyiesespectaclesllistat`
--
...
/*!50001 VIEW `companyiesespectaclesllistat` AS select `c`.`c_idCompanyia` AS `c_idCompanyia`,`c`.`c_Nom` AS `c_Nom`,`c`.`c_Responsable` AS `c_Responsable`,`c`.`c_Telèfon` AS `c_Telèfon`,`c`.`c_Email` AS `c_Email`,`c`.`c_NIF` AS `c_NIF`,`c`.`c_CCC` AS `c_CCC`,`c`.`c_NIFCompanyia` AS `c_NIFCompanyia`,`ep`.`ep_Nom` AS `ep_Nom` from (`companyies` `c` left join `espectacles` `ep` on((`ep`.`ep_idCompanyia` = `c`.`c_idCompanyia`))) */;
...
-- Dump completed on 2018-02-19  8:54:27
