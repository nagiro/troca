...
INSERT INTO `Companyies` VALUES (1,'Companyia la Bleda','Helena Escobar','651300826','yonkis@hoa.cat','789756445','CCC',''),(2,'Companyia 2','hola',NULL,NULL,NULL,NULL,NULL);
...
INSERT INTO `ContracteEspectacles` VALUES (23,11,1,1,100,21,200,21,21,21),(24,12,2,1,100,21,200,21,21,21),(25,12,1,1,100,21,200,21,21,21),(26,12,1,1,100,21,200,21,21,21);
...
INSERT INTO `ContractesControl` VALUES (11,1,1,0,'','2018-03-01',0),(12,1,1,0,'','2018-03-01',0);
...
INSERT INTO `ContractesFuncions` VALUES (24,23,'00:00:00','00:00:00','00:00:00','()','','','','','2018-03-01'),(25,24,'10:00:00','09:00:00','11:00:00','()','','','','','2018-03-13'),(26,25,'00:00:00','00:00:00','00:00:00','()','','','','','2018-03-01');
...
INSERT INTO `Espais` VALUES (1,1,'Aula Magna','Girona',1,0,0,0,'Albert Johé i Martí','651300826','Per aparcar cal anar a la cantonada','Hi ha zona de càrrega i descàrrega davant de la casa però gairebé sempre està ple',1,'Poden canviar-se als camerinos de l\'Auditori Viader.',NULL),(2,2,'Espai la troca','',0,0,0,0,'','','','',0,'','');
...
INSERT INTO `Espectacles` VALUES (1,1,'Espectacle 2','Tècnic 1','Requisit 1','Familiar, circ'),(2,1,'E2','Albert Johé i Martí','Casal amb fort vent','Familiar, dansa'),(22,2,'E3',NULL,NULL,'F3'),(23,2,'E4',NULL,NULL,'F4');
...
INSERT INTO `Preus` VALUES (1,1,100,200,'Prova'),(2,1,200,300,'Més 3 espectacles'),(3,2,100,200,'Amb comisisó');
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `entitatsespais` AS select `e`.`e_idAjuntament` AS `e_idAjuntament`,`e`.`e_Nom` AS `e_Nom`,`e`.`e_Responsable` AS `e_Responsable`,`e`.`e_Telefon` AS `e_Telèfon`,`e`.`e_Email` AS `e_Email`,`e`.`e_Adreca` AS `e_Adreca`,`e`.`e_CodiPostal` AS `e_CodiPostal`,`e`.`e_Ciutat` AS `e_Ciutat`,`e`.`e_CIF` AS `e_CIF`,`es`.`es_idEspai` AS `es_idEspai`,`es`.`es_idAjuntament` AS `es_idAjuntament`,`es`.`es_Nom` AS `es_Nom`,`es`.`es_Poblacio` AS `es_Poblacio`,`es`.`es_TeFitxaTecnica` AS `es_TeFitxaTecnica`,`es`.`es_TeCaixaNegra` AS `es_TeCaixaNegra`,`es`.`es_TeAparcament` AS `es_TeAparcament`,`es`.`es_TeCarregaDescarrega` AS `es_TeCarregaDescarrega`,`es`.`es_Responsable_nom` AS `es_Responsable_nom`,`es`.`es_Responsable_mobil` AS `es_Responsable_mobil`,`es`.`es_Aparcament_Text` AS `es_Aparcament_Text`,`es`.`es_CarregaDescarrega_Text` AS `es_CarregaDescarrega_Text`,`es`.`es_Lloc_canviarse` AS `es_Lloc_canviarse`,`es`.`es_Lloc_Canviarse_text` AS `es_Lloc_Canviarse_text` from (`entitats` `e` left join `espais` `es` on((`es`.`es_idAjuntament` = `e`.`e_idAjuntament`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2018-03-02  7:36:31
